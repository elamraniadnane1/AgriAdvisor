...
def generate_response(question, collection_name):
    ...
    relevant_chunks = cached_query_qdrant(question, collection_name)
...
class Application(tk.Tk):
    ...
    def show_login_window(self):
        ...
        tk.Label(self.login_window, text="Username", font=("Helvetica", 10)).grid(row=0, column=0, padx=10, pady=10)
        tk.Label(self.login_window, text="Password", font=("Helvetica", 10)).grid(row=1, column=0, padx=10, pady=10)

        self.username_entry = tk.Entry(self.login_window, font=("Helvetica", 10))
        self.password_entry = tk.Entry(self.login_window, show='*', font=("Helvetica", 10))

        self.username_entry.grid(row=0, column=1, padx=10, pady=10)
        self.password_entry.grid(row=1, column=1, padx=10, pady=10)

        tk.Button(self.login_window, text="Login", command=self.login, font=("Helvetica", 10)).grid(row=2, column=0, columnspan=2, pady=10)
        tk.Button(self.login_window, text="Register", command=self.register, font=("Helvetica", 10)).grid(row=3, column=0, columnspan=2, pady=10)
    ...
    def process_input(self):
        ...
        if cache_key in self.cache:
            ...
            Thread(target=self.run_user_input_choice, args=(input_lang, output_lang, user_input, "text", cache_key)).start()
    ...
    def transcribe_audio(self):
        ...
        with sr.AudioFile(self.audio_filename) as source:
            ...
            try:
                ...
                self.run_user_input_choice(self.input_lang.get(), self.output_lang.get(), transcription, "voice", cache_key)
            except sr.UnknownValueError:
    ...
    def run_user_input_choice(self, input_lang, output_lang, user_input, input_type, cache_key):
