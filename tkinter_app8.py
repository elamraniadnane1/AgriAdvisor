...
def recognize_speech_from_microphone(language="ar", callback=None):
    ...
    with mic as source:
        ...
        def update_transcription(r, audio):
            try:
                ...
                if callback:
                    callback("Speech was unintelligible")
            except sr.RequestError:
                if callback:
                    callback("Could not request results from Google Speech Recognition service")

        recognizer.listen_in_background(source, update_transcription)
...
class Application(tk.Tk):
    ...
    def show_main_window(self):
        ...
        # Real-time transcription label
        self.transcription_label = ttk.Label(self, text="", foreground="blue")
        self.transcription_label.grid(row=10, column=0, columnspan=3, pady=5)
        self.transcription_label.grid_remove()  # Hide the label initially
    ...
    def process_voice_input(self):
        ...
        # Run the processing in a separate thread
        Thread(target=self.recognize_speech_with_feedback, args=(input_lang, output_lang)).start()

    def recognize_speech_with_feedback(self, input_lang, output_lang):
        recognize_speech_from_microphone(language=input_lang, callback=self.update_transcription_label)

    def update_transcription_label(self, transcription):
        self.transcription_label.config(text=transcription)
        if transcription.lower().strip() == "stop":
